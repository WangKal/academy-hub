...
export default function PublicFooter({ onNavigate }: Props) {
  ...
  return (
    <footer className="border-t transition-theme" style={{ background: 'var(--surface)', borderColor: 'var(--edge)' }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          {sections.map((section) => (
            <div key={section.title}>
              <h4 className="text-xs font-semibold uppercase tracking-widest text-ink-4 mb-3">{section.title}</h4>
        ...
        <div className="mt-12 pt-6 border-t flex flex-col sm:flex-row items-center justify-between gap-4" style={{ borderColor: 'var(--edge)' }}>
