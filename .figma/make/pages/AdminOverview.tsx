import { adminStats, adminPayments, auditLogs, courses, relativeTime, formatCurrency } from '@/lib/data'
import { PageHeader, MetricCard, Badge, Btn, BarChart, DataTable, type Column } from '@/components/ui'
...
export default function AdminOverview({ onNavigate }: Props) {
  const paymentCols: Column<Payment>[] = [
    ...
    },
    {
      ...
      label: 'Status',
      render: (row) => {
        const v = { succeeded: 'success', pending: 'warning', failed: 'danger', refunded: 'neutral' } as const
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <PageHeader
        ...
        actions={<Badge variant="success"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block mr-1" />All systems operational</Badge>}
      />
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          {adminStats.pendingVerifications > 0 && (
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5">
              <span className="text-amber-600 text-sm font-semibold">{adminStats.pendingVerifications}</span>
              <span className="text-amber-700 text-sm">pending email verifications</span>
              <Btn variant="ghost" size="sm" onClick={() => onNavigate('admin-users')}>Review →</Btn>
            </div>
          )}
          {adminStats.openSupportTickets > 0 && (
            <div className="flex items-center gap-2 bg-sky-50 border border-sky-200 rounded-lg px-4 py-2.5">
              <span className="text-sky-600 text-sm font-semibold">{adminStats.openSupportTickets}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 bg-white border border-stone-200 rounded-xl p-5">
          ...
          <BarChart
            data={adminStats.revenueByMonth.map((d) => ({ ...d, display: Math.round(d.amount / 1000) }))}
            valueKey="amount"
            labelKey="month"
            color="#4f46e5"
          />
        ...
        <div className="space-y-4">
          <div className="bg-white border border-stone-200 rounded-xl p-5">
            <h3 className="text-xs font-semibold text-stone-700 uppercase tracking-wider mb-4">Platform health</h3>
            <div className="space-y-3">
              {[
                { label: 'Course catalogue', value: `${adminStats.totalCourses} courses`, status: 'healthy' },
                { label: 'Published courses', value: `${courses.filter((c) => c.published).length} active`, status: 'healthy' },
                ...
                <div key={item.label} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'healthy' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
          ...
          <div className="bg-white border border-stone-200 rounded-xl p-5">
            ...
            <div className="flex flex-col gap-2">
              <Btn variant="secondary" size="sm" className="justify-start" onClick={() => onNavigate('admin-users')}>
                Manage users
              </Btn>
              <Btn variant="secondary" size="sm" className="justify-start" onClick={() => onNavigate('admin-payments')}>
                Review payments
              </Btn>
              <Btn variant="secondary" size="sm" className="justify-start" onClick={() => onNavigate('admin-certificates')}>
                Issue certificate
              </Btn>
              <Btn variant="secondary" size="sm" className="justify-start" onClick={() => onNavigate('admin-audit')}>
                View audit log
              </Btn>
            </div>
          </div>
      ...
      <div className="bg-white border border-stone-200 rounded-xl mb-6 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
          ...
          <Btn variant="ghost" size="sm" onClick={() => onNavigate('admin-payments')}>View all →</Btn>
        </div>
      ...
      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
          <h2 className="font-display text-base font-semibold text-stone-900">Recent audit events</h2>
          <Btn variant="ghost" size="sm" onClick={() => onNavigate('admin-audit')}>View all →</Btn>
        </div>
...
function DollarIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="5.5" stroke="currentColor" strokeWidth="1.3"/><path d="M8 4.5v7M6 6.5c0-1 1-1.5 2-1.5s2 .5 2 1.5S9 8 8 8s-2 .5-2 1.5S7 11 8 11s2-.5 2-1.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
}
