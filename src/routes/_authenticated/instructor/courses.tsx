...
export const Route = createFileRoute("/_authenticated/instructor/courses")({
  beforeLoad: ({ context }) => {
    ...
    if (!user || !(
      can(user, "courses", "view", { instructorId: user.id }) ||
      (user.role === "admin" && can(user, "courses", "view"))
    )) {
...
function InstructorCourses() {
  const { user } = useAuth();
  const canCreate = !!user && can(user, "courses", "create", { instructorId: user.id });
  const canPublish = !!user && can(user, "courses", "publish", { instructorId: user.id });
  const canEdit = !!user && can(user, "courses", "edit", { instructorId: user.id });
  const qc = useQueryClient();
  ...
  return (
    <AppShell title="My Teaching" description="Courses you teach and the curriculum you manage" actions={canCreate ? <NewCourseDialog /> : undefined}>
      {isLoading ? (
        <LoadingBlock />
      ) : error ? (
                ...
                <TableRow key={c.id}>
                  ...
                  <TableCell className="space-x-2 text-right">
                    <Button asChild size="sm" variant="ghost">
                      {canEdit && <Link to="/instructor/courses/$courseId" params={{ courseId: c.id }}>
                        Edit
                      </Link>}
                    </Button>
                    <Button
