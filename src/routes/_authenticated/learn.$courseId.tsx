...
import { Link, createFileRoute, useParams } from "@/lib/router";
...
function Player() {
  ...
  const { data: activeLessonRecord } = useQuery({
    queryKey: ["active-lesson", activeId],
    queryFn: () => api.getLesson(activeId!),
    enabled: !!activeId,
  });
  ...
  const active = activeLessonRecord ?? lessons.find((l) => l.id === activeId) ?? null;
  const completed = new Set(
    (progress ?? []).filter((p) => p.status === "completed").map((p) => p.lessonId),
  );

  const canComplete = !!user && can(user, "learning", "complete", { studentId: user.id, enrolledStudentId: user.id });
              ...
              {active.lessonType === "quiz" ? (
                ...
                <>
                  ...
                  {!!active.attachments?.length && (
                    <div className="mt-8 rounded-lg border border-edge bg-card p-4">
                      <h2 className="font-display text-sm font-semibold">Lesson resources</h2>
                      <ul className="mt-3 space-y-2">
                        {active.attachments.map((attachment) => (
                          <li key={attachment.url}>
                            <a href={attachment.url} target="_blank" rel="noreferrer" className="text-sm text-primary hover:underline">
                              {attachment.name}
                            </a>
                          </li>
                        ))}
                  ...
                  <div className="mt-10 flex items-center gap-3 border-t border-edge pt-6">
                    <Button
                      disabled={!canComplete || complete.isPending || completed.has(active.id)}
