  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-stone-50">
      ...
      <div className="bg-white border-b border-stone-200 px-5">
        <TabBar tabs={tabs} active={tab} onChange={(id) => setTab(id as BuilderTab)} />
      </div>
          ...
          <div>
            ...
            <select className="w-full px-3 py-2 text-sm bg-white border border-stone-200 rounded-lg text-stone-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
              {["Engineering", "Data Science", "Design", "AI & ML", "Cloud"].map((c) => (
                <option key={c} selected={c === course.category}>
                  {c}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <select className="w-full px-3 py-2 text-sm bg-white border border-stone-200 rounded-lg text-stone-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
              {["Beginner", "Intermediate", "Advanced"].map((l) => (
                <option key={l} selected={l === course.level}>
                  {l}
                </option>
              ))}
            </select>
          ...
          <div className="space-y-2">
            {course.outcomes.map((outcome, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <button className="text-stone-400 hover:text-rose-500 transition-colors text-lg leading-none">
                  ×
                </button>
              </div>
            ))}
            ...
            return (
              <div key={mod.id} className="rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <span
                    className={`flex-1 text-xs font-semibold transition-colors ${isOpen ? "text-indigo-700" : "text-stone-700"}`}
                  >
                    {mod.order}. {mod.title}
                  </span>
                  <span className="text-[10px] font-mono text-stone-400">{mod.lessons.length}</span>
                  <span
                    className={`text-stone-400 transition-transform text-xs ${isOpen ? "rotate-180" : ""}`}
                  >
                    ▾
                  </span>
                </button>
                    ...
                    {mod.lessons.map((lesson) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`flex-1 text-xs truncate ${isSelected ? "text-indigo-700 font-medium" : "text-stone-600"}`}
                          >
                            {lesson.title}
                          </span>
...
function QuizzesTab({ course }: { course: Course }) {
  const quizLessons = course.modules.flatMap((m) =>
    m.lessons.filter((l) => l.type === "quiz").map((l) => ({ ...l, module: m.title })),
  );
      ...
      <div className="max-w-2xl">
        ...
        <div className="space-y-3">
          {quizLessons.map((quiz) => (
            <div
              key={quiz.id}
              className="bg-white border border-stone-200 rounded-xl p-4 hover:border-stone-300 transition-colors cursor-pointer"
            >
        ...
        <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100">
          {[
            { label: "Allow enrollment", desc: "Students can self-enroll", on: true },
            {
              ...
              desc: "Issue certificate when course is completed",
              on: true,
            },
            {
              label: "Sequential lesson access",
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden border border-stone-200">
        ...
        <div className="p-5">
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <button className="bg-indigo-600 text-white text-sm px-4 py-2 rounded-lg font-medium">
              Enroll now
            </button>
          </div>
        </div>
