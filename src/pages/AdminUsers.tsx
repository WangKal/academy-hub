...
import { PageHeader, Badge, Avatar, Btn, Input, Select, DataTable, type Column, ConfirmDialog } from '@/components/ui'
...
export default function AdminUsers({ onNavigate }: Props) {
  ...
  const [confirmAction, setConfirmAction] = useState<{ type: 'suspend' | 'delete'; user: User } | null>(null)
  ...
  const columns: Column<User>[] = [
    ...
    },
    {
      ...
      label: 'Role',
      render: (row) => (
        <Badge variant={row.role === 'admin' ? 'info' : row.role === 'instructor' ? 'default' : 'neutral'}>
          {row.role}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      label: 'Joined',
      mono: true,
      render: (row) => new Date(row.joinedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
    },
    {
      key: 'enrollments',
      label: 'Enrolled',
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); setConfirmAction({ type: 'suspend', user: row }) }}
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        {[
          ...
          { label: 'Active', count: adminUsers.filter((u) => u.status === 'active').length, color: 'text-emerald-600' },
          { label: 'Suspended', count: adminUsers.filter((u) => u.status === 'suspended').length, color: 'text-rose-600' },
          { label: 'Pending', count: adminUsers.filter((u) => u.status === 'pending').length, color: 'text-amber-600' },
          { label: 'Instructors', count: adminUsers.filter((u) => u.role === 'instructor').length, color: 'text-indigo-600' },
        ].map((s) => (
      ...
      <div className="bg-white border border-stone-200 rounded-xl p-4 mb-5 flex flex-wrap items-center gap-3">
        ...
        <span className="ml-auto text-xs text-stone-400 font-mono">{filtered.length} result{filtered.length !== 1 ? 's' : ''}</span>
      </div>
      ...
      <div className="flex items-center justify-between mt-4 px-1">
        <span className="text-xs text-stone-400 font-mono">Showing {filtered.length} of {adminUsers.length} users</span>
      ...
      {/* User detail panel */}
      {selectedUser && (
        <UserDetailPanel user={selectedUser} onClose={() => setSelectedUser(null)} onAction={(type) => setConfirmAction({ type, user: selectedUser })} />
      )}

      {/* Confirm dialog */}
      <ConfirmDialog
        open={!!confirmAction}
        title={confirmAction?.type === 'suspend' ? `${confirmAction.user.status === 'suspended' ? 'Unsuspend' : 'Suspend'} user` : 'Delete user'}
        body={
        ...
        confirmLabel={confirmAction?.type === 'suspend' ? (confirmAction.user.status === 'suspended' ? 'Unsuspend' : 'Suspend') : 'Delete'}
...
function UserDetailPanel({ user, onClose, onAction }: { user: User; onClose: () => void; onAction: (type: 'suspend' | 'delete') => void }) {
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="bg-stone-50 rounded-xl p-4 space-y-3 text-sm">
            {[
              ...
              { label: 'Status', value: <Badge variant={user.status === 'active' ? 'success' : user.status === 'suspended' ? 'danger' : 'warning'}>{user.status}</Badge> },
          ...
          <div className="flex flex-col gap-2">
            <Btn variant="secondary" size="sm" className="w-full justify-center">Edit profile</Btn>
            <Btn variant="secondary" size="sm" className="w-full justify-center" onClick={() => onAction('suspend')}>
              {user.status === 'suspended' ? 'Unsuspend account' : 'Suspend account'}
            </Btn>
            <Btn variant="danger" size="sm" className="w-full justify-center" onClick={() => onAction('delete')}>
              Delete account
            </Btn>
          </div>
        </div>
