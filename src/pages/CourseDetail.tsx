  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      <nav className="flex items-center gap-1.5 text-xs text-stone-400 mb-6">
        <button onClick={() => onNavigate('catalogue')} className="hover:text-stone-600 transition-colors">Catalogue</button>
        <span>/</span>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              {course.published ? <Badge variant="success">Published</Badge> : <Badge variant="warning">Draft</Badge>}
            </div>
            <h1 className="font-display text-3xl font-semibold text-stone-900 leading-tight mb-3">{course.title}</h1>
            ...
            <div className="flex flex-wrap items-center gap-4 mt-4 text-sm text-stone-500">
              <span>⭐ <strong className="text-stone-800">{course.rating}</strong> rating</span>
              <span>·</span>
              <span><strong className="text-stone-800">{course.enrolled.toLocaleString()}</strong> students enrolled</span>
              <span>·</span>
              <span><strong className="text-stone-800">{course.totalLessons}</strong> lessons</span>
              <span>·</span>
              <span><strong className="text-stone-800">{formatDuration(course.totalDuration)}</strong> total</span>
            </div>

            <div className="flex items-center gap-2 mt-4">
              <Avatar name={course.instructor.name} size="sm" />
              <span className="text-sm text-stone-600">Instructor: <span className="font-medium text-stone-800">{course.instructor.name}</span></span>
            </div>
          </div>

          {/* Cover image */}
          <div className="relative rounded-2xl overflow-hidden">
            <img src={course.coverImage} alt={course.title} className="w-full h-56 object-cover bg-stone-100" />
          ...
          {course.outcomes.length > 0 && (
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-5">
              <h2 className="font-display text-lg font-semibold text-stone-900 mb-4">What you will learn</h2>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-xs text-stone-400 font-mono">
                  {totalModules} modules · {course.totalLessons} lessons · {formatDuration(course.totalDuration)}
                </div>
              </div>
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-stone-400 mt-0.5 font-mono">
                            ...
                            {completedCount > 0 && ` · ${completedCount}/${mod.lessons.length} done`}
                          </div>
                        </div>
                            ...
                            return (
                              ...
                              >
                                <div className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 ${
                                  completed ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-stone-300'
                                }`}>
                                  {completed ? <span className="text-[10px]">✓</span> : <LessonTypeIcon type={lesson.type} />}
                                </div>
                                <div className="flex-1">
                                  <div className={`text-xs font-medium ${isCurrent ? 'text-indigo-700' : 'text-stone-700'}`}>
                                    {lesson.title}
                                    {isCurrent && <span className="ml-2 text-[10px] bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded-full font-mono">Current</span>}
                                  </div>
                                </div>
          ...
          <div className="sticky top-6">
            ...
            <div className="mt-4 bg-white border border-stone-200 rounded-xl p-4">
              ...
              <div className="space-y-2 text-xs text-stone-600">
                <div className="flex justify-between"><span>📹 Video lessons</span><span className="font-mono">{course.totalLessons}</span></div>
                <div className="flex justify-between"><span>⏱ Total duration</span><span className="font-mono">{formatDuration(course.totalDuration)}</span></div>
                <div className="flex justify-between"><span>📋 Modules</span><span className="font-mono">{course.modules.length}</span></div>
                <div className="flex justify-between"><span>📜 Certificate</span><span className="font-mono text-emerald-600">Yes</span></div>
                <div className="flex justify-between"><span>♾ Access</span><span className="font-mono">Lifetime</span></div>
              </div>
            </div>
          </div>
        </div>
...
function EnrolledState({ enrollment, course, onNavigate }: { enrollment: ReturnType<typeof studentEnrollments.find>; course: ReturnType<typeof getCourseById>; onNavigate: (page: string, params?: Record<string, string>) => void }) {
  ...
  return (
    <>
      ...
      <div className="mt-4 flex flex-col gap-2">
        <Btn className="w-full" onClick={() => onNavigate('course-player', { courseId: course.id })}>
...
function PurchaseState({ course, enrolling, onEnroll }: { course: NonNullable<ReturnType<typeof getCourseById>>; enrolling: boolean; onEnroll: () => void }) {
  return (
    <>
      ...
      <Btn variant="secondary" size="sm" className="w-full">Add to wishlist</Btn>
