    ...
    >
      <div className="w-full max-w-sm text-center">
        ...
        <button
          ...
          className="flex items-center gap-2 mb-10 mx-auto w-fit"
        >
        ...
        {currentStatus === "pending" && (
          <PendingState onResend={() => setCurrentStatus("success")} />
        )}
        {currentStatus === "success" && <SuccessState onSignIn={onSignIn} />}
        {currentStatus === "expired" && (
          <ExpiredState onResend={() => setCurrentStatus("pending")} onNavigate={onNavigate} />
        )}
  ...
  return (
    <>
      ...
      <p className="text-sm text-ink-3 mb-6">
        Your Academy Hub account is ready — you just need to verify your email address. Check your
      ...
      >
        {[
          ...
          <div key={i} className="flex items-center gap-2.5 text-xs text-ink-2">
            <span className="w-5 h-5 rounded-full bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-[10px] font-bold shrink-0">
              {i + 1}
            </span>
            {step}
          </div>
...
function SuccessState({ onSignIn }: { onSignIn: () => void }) {
  return (
    <>
      ...
      <p className="text-sm text-ink-3 mb-6">
        Your account is now active. Welcome to Academy Hub — your learning journey begins now.
      </p>
      <button
        onClick={onSignIn}
...
function ExpiredState({
  ...
  return (
    <>
      <div className="w-14 h-14 rounded-2xl bg-rose-50 dark:bg-rose-950 border border-rose-100 dark:border-rose-900 flex items-center justify-center text-2xl mx-auto mb-5">
        ⏱
      </div>
      <h2 className="font-display text-xl font-semibold text-ink-1 mb-2">Link expired</h2>
      <p className="text-sm text-ink-3 mb-6">
        This verification link has expired. Verification links are valid for 24 hours. Request a new
        one below.
      </p>
      <button
        onClick={onResend}
...
function InvalidState({ onNavigate }: { onNavigate: (p: string) => void }) {
  return (
    <>
      ...
      <p className="text-sm text-ink-3 mb-6">
        This verification link is invalid or has already been used. If you believe this is an error,
