...
export default function HomePage({ onNavigate, onSignIn }: Props) {
  ...
  const featured = published.find((c) => c.enrolled > 800)
  const heroCards = published.slice(0, 3)
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative max-w-7xl mx-auto px-6 sm:px-8 pt-20 pb-24 lg:pt-24 lg:pb-32">
          <div className="grid lg:grid-cols-[1fr_440px] gap-12 lg:gap-8 items-center">
            ...
            <div>
              ...
              {/* Headline — Fraunces at maximum scale */}
              <h1
              ...
              <div className="flex flex-wrap gap-3 mb-12">
                <button
                  ...
                  className="px-6 py-3 font-semibold text-sm text-white rounded-xl transition-all hover:opacity-90 active:scale-95"
                ...
                <button
                  ...
                  className="px-6 py-3 font-semibold text-sm rounded-xl border transition-all hover:bg-white/5"
            ...
            <div className="hidden lg:block relative h-[480px] select-none">
              {heroCards.map((course, i) => {
                ...
                return (
                  <div
                    key={course.id}
                    className="absolute w-[240px] rounded-2xl overflow-hidden transition-all duration-500 hover:scale-105 hover:rotate-0 cursor-pointer"
                    style={{
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-mono" style={{ color: '#64748b' }}>⭐ {course.rating}</span>
      ...
      {/* ─── Section divider: scrolling marquee stats strip ─────────────────── */}
      <div
      ...
      <section className="py-16 sm:py-24" style={{ background: 'var(--bg)' }}>
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 mb-10">
            ...
            >
              All {published.length} courses →
            </button>
          </div>

          {/* Category tabs — archival underline style */}
          ...
          <div className="max-w-md mb-8">
            <div className="relative">
              ...
              />
              {query && (
                <button onClick={() => setQuery('')} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-ink-4 hover:text-ink-2 text-base leading-none">×</button>
              )}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
              {filtered.slice(0, 6).map((course, i) => (
                <PublicCourseCard
                  ...
                  tall={i === 0 && activeCategory === 'All' && !query}
                />
              ))}
            </div>
          ) : (
      ...
      {/* ─── 02 — Features: archival numbered list ───────────────────────────── */}
      <section
      ...
      >
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          <div className="grid lg:grid-cols-[300px_1fr] gap-12 lg:gap-24">
            ...
            {/* Right: numbered feature list — editorial rows */}
            ...
            <div className="grid lg:grid-cols-2 rounded-2xl overflow-hidden border" style={{ borderColor: 'var(--edge)' }}>
              ...
              <div className="relative overflow-hidden" style={{ minHeight: 280 }}>
                ...
                <div className="absolute bottom-5 left-5 right-5">
                  <div className="text-white font-mono text-xs" style={{ color: 'rgba(255,255,255,0.7)' }}>
                    ⭐ {featured.rating} · {featured.enrolled.toLocaleString()} students
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col justify-center p-7 sm:p-10" style={{ background: 'var(--surface)' }}>
                ...
                <p className="text-sm text-ink-3 leading-relaxed mb-6">{featured.description}</p>
                <div className="flex flex-wrap gap-4 text-xs font-mono text-ink-4 mb-8 pb-8 border-b" style={{ borderColor: 'var(--edge)' }}>
                  ...
                  <span>By {featured.instructor.name}</span>
                </div>
                <div className="flex items-center gap-5">
                  <button
                    onClick={() => onNavigate('course-detail', { courseId: featured.id })}
                    className="px-5 py-2.5 bg-indigo-600 text-white font-semibold text-sm rounded-xl hover:bg-indigo-700 transition-all"
                  >
                    View course
                  </button>
                  <span className="font-display text-2xl font-semibold text-ink-1">${featured.price}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ─── 05 — How it works: horizontal timeline ──────────────────────────── */}
      <section className="py-16 sm:py-24" style={{ background: 'var(--bg)' }}>
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
...
function PublicCourseCard({
  ...
  return (
    <button
      onClick={() => onNavigate('course-detail', { courseId: course.id })}
      className="text-left rounded-2xl overflow-hidden border transition-all duration-300 group hover:shadow-xl hover:-translate-y-1"
      style={{ background: 'var(--surface)', borderColor: 'var(--edge)' }}
    >
      <div className="relative overflow-hidden">
        <img
        ...
        <div className="absolute top-3 left-3">
          <span className={`text-[10px] font-mono font-semibold px-2 py-0.5 rounded-full ${levelColors[course.level] ?? ''}`}>
            {course.level}
          </span>
        </div>
        {/* Price on hover */}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between text-xs border-t pt-3" style={{ borderColor: 'var(--edge)' }}>
          ...
          <span className="font-bold text-ink-1 opacity-100 group-hover:opacity-0 transition-opacity">${course.price}</span>
        </div>
      </div>
    </button>
  )
