...
export default function ForgotPassword({ onNavigate }: Props) {
    ...
    return (
      <div className={container} style={{ background: 'var(--bg)' }}>
        <div className="w-full max-w-sm text-center">
          ...
          <div className="w-14 h-14 rounded-2xl bg-indigo-50 dark:bg-indigo-950 border border-indigo-100 dark:border-indigo-900 flex items-center justify-center text-2xl mx-auto mb-5">📬</div>
          <h2 className="font-display text-xl font-semibold text-ink-1 mb-2">Check your email</h2>
          <p className="text-sm text-ink-3 mb-6">
            We sent a password reset link to <strong className="text-ink-1">{email}</strong>. The link expires in 15 minutes.
          </p>
          ...
          <p className="text-xs text-ink-4 mt-4">Didn't receive it? Check your spam folder or{' '}
            <button onClick={() => setStep('email')} className="text-indigo-600 hover:text-indigo-700">try again</button>.
          </p>
        </div>
      </div>
    )
    ...
    return (
      <div className={container} style={{ background: 'var(--bg)' }}>
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-ink-3 mb-6">Choose a strong password for your Academy Hub account.</p>
          ...
          <form onSubmit={handleReset} className="space-y-4" noValidate>
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-2 mb-1.5">Confirm password</label>
              <input
                type="password"
    ...
    return (
      <div className={container} style={{ background: 'var(--bg)' }}>
        <div className="w-full max-w-sm text-center">
          ...
          <p className="text-sm text-ink-3 mb-6">Your password has been successfully reset. You can now sign in with your new password.</p>
          <button
    ...
    return (
      <div className={container} style={{ background: 'var(--bg)' }}>
        <div className="w-full max-w-sm text-center">
          ...
          <p className="text-sm text-ink-3 mb-6">This reset link is no longer valid. Reset links expire after 15 minutes for security.</p>
          <button
          ...
          <button onClick={() => onNavigate('login')} className="text-xs text-ink-3 hover:text-ink-1">
            ← Back to sign in
          </button>
        </div>
      </div>
  ...
  return (
    <div className={container} style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-sm">
        <BackToHome onNavigate={onNavigate} />
        <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950 border border-indigo-100 dark:border-indigo-900 flex items-center justify-center text-lg mb-5">🔑</div>
        <h1 className="font-display text-xl font-semibold text-ink-1 mb-1">Forgot your password?</h1>
        ...
        <p className="text-center text-xs text-ink-3 mt-5">
          ...
          <button onClick={() => onNavigate('login')} className="text-indigo-600 font-semibold hover:text-indigo-700">Sign in</button>
        </p>
      </div>
    </div>
  )
