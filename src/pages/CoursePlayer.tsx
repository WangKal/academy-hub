...
export default function CoursePlayer({ courseId, onNavigate }: Props) {
  ...
  const progress =
    allLessons.length > 0 ? Math.round((completed.size / allLessons.length) * 100) : 0;
    ...
    <div className="flex h-full overflow-hidden bg-stone-950">
      ...
      >
        ...
        <div className="p-4 border-b border-gray-800 shrink-0">
          ...
          <div className="text-sm font-semibold text-gray-100 leading-snug mb-2">
            {course.title}
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div
                          className={`text-xs leading-snug ${isActive ? "text-indigo-200 font-medium" : isDone ? "text-gray-500 line-through" : "text-gray-300"}`}
                        >
                          {lesson.title}
                        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      <div className="prose prose-invert prose-sm max-w-none text-gray-300 space-y-4">
        <p>
          This lesson covers the core principles of the topic, with practical examples from
          production codebases. As you work through this material, consider how these patterns apply
          to your own projects.
        </p>
        <h3 className="text-gray-100 font-semibold text-base mt-6">Core concepts</h3>
        <p>
          The fundamental insight here is that composition is more powerful than inheritance in most
          UI scenarios. When you build components with clear, narrow interfaces, they become far
        ...
        <p>
          This pattern is used extensively in modern design systems — Radix UI, shadcn/ui, and
          Headless UI all follow variants of this approach. By the end of this module, you will have
          the vocabulary and intuition to read and contribute to these libraries confidently.
        </p>
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-lg border text-sm transition-all ${
                    isCorrect
                      ...
                      : isWrong
                        ? "bg-rose-900/30 border-rose-600 text-rose-300"
                        : isSelected
      ...
      {!submitted ? (
        ...
        <div
          className={`mt-4 p-4 rounded-xl border ${selected === questions[0].correct ? "bg-emerald-900/20 border-emerald-700" : "bg-rose-900/20 border-rose-700"}`}
        >
          <div
            className={`font-semibold text-sm ${selected === questions[0].correct ? "text-emerald-400" : "text-rose-400"}`}
          >
...
function LessonTypePill({ type }: { type: "video" | "text" | "quiz" }) {
  ...
  return (
    <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${cfg} capitalize`}>{type}</span>
  );
}
