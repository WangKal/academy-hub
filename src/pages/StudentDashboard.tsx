...
import {
  courses, studentEnrollments, studentStats, certificates, notifications,
  formatDuration, relativeTime, type Enrollment, type Course,
} from '@/lib/data'
import {
  PageHeader, Badge, ProgressBar, MetricCard, Avatar, RingProgress, Card, Btn,
} from '@/components/ui'
...
export default function StudentDashboard({ onNavigate }: Props) {
  ...
  const primaryCourse = primaryEnrollment ? courses.find((c) => c.id === primaryEnrollment.courseId) : null
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="mb-6 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <span className="text-sm text-indigo-600 ml-2">
              You have {studentStats.weeklyGoal - studentStats.weeklyProgress} lesson{studentStats.weeklyGoal - studentStats.weeklyProgress !== 1 ? 's' : ''} left to hit your weekly goal.
            </span>
          </div>
          <button onClick={() => setDismissedTip(true)} className="text-indigo-400 hover:text-indigo-600 text-lg leading-none">×</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div>
            <h2 className="font-display text-lg font-semibold text-stone-900 mb-3">Continue learning</h2>
          ...
          {inProgress.length > 1 && (
            <div>
              <h2 className="font-display text-lg font-semibold text-stone-900 mb-3">Also in progress</h2>
            ...
            <div>
              ...
              <div className="space-y-3">
                {completed.map((enrollment) => {
                  ...
                  if (!course) return null
                  return (
                    <div key={enrollment.courseId} className="bg-white border border-stone-200 rounded-xl p-4 flex items-center gap-4">
                      <img src={course.coverImage} alt={course.title} className="w-14 h-10 rounded-lg object-cover bg-stone-100" />
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-stone-800 truncate">{course.title}</div>
                        <div className="text-xs text-stone-400 mt-0.5">
                          Completed {enrollment.completedAt ? new Date(enrollment.completedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : ''}
                        </div>
                      </div>
        ...
        <div className="space-y-5">
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-stone-400 font-mono">{studentStats.weeklyProgress}/{studentStats.weeklyGoal} lessons</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="relative">
                <RingProgress value={(studentStats.weeklyProgress / studentStats.weeklyGoal) * 100} size={64} stroke={6} />
              ...
              <div>
                <div className="text-sm font-medium text-stone-800">{studentStats.weeklyProgress} done</div>
                <div className="text-xs text-stone-400">{studentStats.weeklyGoal - studentStats.weeklyProgress} remaining</div>
                <div className="text-xs text-indigo-600 mt-1">{studentStats.currentStreak} day streak 🔥</div>
              </div>
            </div>
          </Card>
              ...
              {certificates.slice(0, 1).map((cert) => (
                <div key={cert.id}>
                  <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-lg p-3 mb-3">
                    <div className="text-xs font-semibold text-indigo-800 mb-0.5">{cert.courseTitle}</div>
                  ...
                  <Btn variant="secondary" size="sm" className="w-full" onClick={() => onNavigate('certificates')}>
          ...
          <Card className="p-5">
            ...
            <div className="space-y-3">
              {courses.filter((c) => !studentEnrollments.find((e) => e.courseId === c.id)).slice(0, 2).map((course) => (
                <button
                  key={course.id}
                ...
                >
                  ...
                  />
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium text-stone-800 group-hover:text-indigo-700 transition-colors line-clamp-2">{course.title}</div>
                    <div className="text-[10px] text-stone-400 mt-0.5">{course.instructor.name} · ${course.price}</div>
                  </div>
                </button>
              ))}
            </div>
            <Btn variant="ghost" size="sm" className="w-full mt-3" onClick={() => onNavigate('catalogue')}>
              Browse all →
            </Btn>
          </Card>
              ...
              {notifications.slice(0, 3).map((n) => (
                <div key={n.id} className="flex gap-2.5 items-start">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-stone-400 mt-0.5">{relativeTime(n.createdAt)}</div>
                  </div>
                </div>
              ))}
            </div>
...
function ContinueLearningCard({
  course, enrollment, nextLesson, onContinue,
}: { course: Course; enrollment: Enrollment; nextLesson?: string; onContinue: () => void }) {
  ...
  return (
    <div className="bg-white border border-stone-200 rounded-xl overflow-hidden hover:border-stone-300 hover:shadow-sm transition-all">
      <div className="relative">
        <img src={course.coverImage} alt={course.title} className="w-full h-36 object-cover bg-stone-100" />
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-sm font-medium text-stone-800">{currentModule?.title ?? 'Getting started'}</div>
          </div>
...
function CompactCourseRow({
  course, enrollment, onClick,
}: { course: Course; enrollment: Enrollment; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      <img src={course.coverImage} alt={course.title} className="w-12 h-10 rounded-lg object-cover bg-stone-100 shrink-0" />
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-stone-800 truncate">{course.title}</div>
        {enrollment.lastLessonTitle && (
          <div className="text-xs text-stone-400 mt-0.5 truncate">Last: {enrollment.lastLessonTitle}</div>
        )}
...
function PlayIcon() {
  return <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 2l9 5-9 5V2Z" fill="currentColor" /></svg>
}
function CoursesIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><line x1="5" y1="3" x2="5" y2="13" stroke="currentColor" strokeWidth="1.3"/><line x1="8" y1="6" x2="12" y2="6" stroke="currentColor" strokeWidth="1.3"/><line x1="8" y1="9" x2="12" y2="9" stroke="currentColor" strokeWidth="1.3"/></svg>
}
function CheckIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="5.5" stroke="currentColor" strokeWidth="1.3"/><path d="M5.5 8l2 2 3-3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
}
function ClockIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="5.5" stroke="currentColor" strokeWidth="1.3"/><path d="M8 5v3.5l2.5 1.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
}
function AwardIcon() {
