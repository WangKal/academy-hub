...
export function Avatar({ name, size = "md", src }: AvatarProps) {
  ...
  return (
    <div
      className={`${sizeClass} ${palette} rounded-full flex items-center justify-center font-semibold shrink-0 select-none`}
    >
      {initials(name)}
    </div>
  );
...
export function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ring-1 font-mono ${badgeClasses[variant]} ${className}`}
    >
      {children}
    </span>
  );
...
export function ProgressBar({
  ...
  return (
    <div className={`flex items-center gap-2 w-full ${className}`}>
      ...
      {showLabel && (
        <span className="text-xs font-mono text-stone-500 shrink-0 w-8 text-right">
          {Math.round(pct)}%
        </span>
      )}
    </div>
...
export function MetricCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-5 flex flex-col gap-3 hover:border-stone-300 transition-colors">
      ...
      {trend && (
        <div
          className={`flex items-center gap-1 text-xs font-medium ${trend.value >= 0 ? "text-emerald-600" : "text-rose-600"}`}
        >
          <span>
        ...
        <nav className="flex items-center gap-1.5 text-xs text-stone-400 mb-1">
          {breadcrumb.map((crumb, i) => (
            <span key={i} className="flex items-center gap-1.5">
              {i > 0 && <span>/</span>}
              {crumb.onClick ? (
                <button onClick={crumb.onClick} className="hover:text-stone-600 transition-colors">
                  {crumb.label}
                </button>
              ) : (
    ...
    <div className="flex gap-0 border-b border-stone-200 mb-6">
      {tabs.map((tab) => (
        ...
        >
          ...
          {tab.count !== undefined && (
            <span
              className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full font-mono ${active === tab.id ? "bg-indigo-100 text-indigo-600" : "bg-stone-100 text-stone-500"}`}
            >
              {tab.count}
            </span>
          )}
          ...
          {rows.map((row) => (
            ...
            >
              {columns.map((col) => (
                <td
                  key={col.key}
                  className={`px-4 py-3 text-stone-700 ${col.mono ? "font-mono text-xs" : ""}`}
                >
                  {col.render
                    ? col.render(row)
