...
export function AssignmentPanel({ lessonId }: { lessonId: string }) {
  ...
  const { data: assignment } = useQuery({
    ...
    queryFn: () => api.getAssignment(lessonId),
  });
      ...
      {graded ? (
        ...
        <>
          {assignment?.instructionsHtml && (
            <div
              className="prose prose-sm mt-4 max-w-none text-ink-3"
          ...
          <Button disabled={submit.isPending || !assignment} onClick={() => submit.mutate()}>
