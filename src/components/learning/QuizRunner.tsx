...
export function QuizRunner({ lessonId, onPassed }: { lessonId: string; onPassed?: () => void }) {
  ...
  const { data: attempts = [] } = useQuery({
    ...
    queryFn: () => api.getQuizAttempts(quiz!.id),
    enabled: !!quiz,
  });
  ...
  return (
    <div className="mt-6">
      ...
      {attempts.length > 0 && <div className="mb-6 rounded-lg border border-edge bg-surface-2 p-4">
        <p className="text-sm font-semibold">Previous attempts</p>
        <div className="mt-2 flex flex-wrap gap-2">
          {attempts.slice(-5).reverse().map((attempt) => <span key={attempt.id} className="rounded-full border border-edge bg-card px-3 py-1 text-xs">{attempt.scorePercent}% · {attempt.passed ? "Passed" : "Not passed"}</span>)}
        </div>
      </div>}
